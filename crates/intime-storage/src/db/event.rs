...
impl EventRepository for SqliteRepository {
    async fn add_event(&self, event: &Event, app_id: Option<i64>, screenshot_path: Option<String>) -> Result<i64, StorageError> {
        let event_name = event.data.name();
        let occured_at = event.timestamp.as_datetime();
        let payload = match &event.data {
            // No-payload variants
            EventData::IdleStart | EventData::IdleEnd | EventData::Gap => None,
            // Variants that require payload
            _ => Some(
                serde_json::to_string(&event.data).map_err(|e| StorageError::Serialization(e))?,
            ),
        };
